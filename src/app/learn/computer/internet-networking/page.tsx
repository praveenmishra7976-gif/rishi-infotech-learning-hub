import Link from "next/link";

const networkTypes = [
  {
    icon: "🏠",
    title: "LAN",
    full: "Local Area Network",
    description:
      "A network that connects computers and devices within a small geographical area such as a home, school, office or laboratory.",
    example: "Office or school network",
  },
  {
    icon: "🌎",
    title: "WAN",
    full: "Wide Area Network",
    description:
      "A network that covers a large geographical area and can connect networks across cities, countries or continents.",
    example: "Internet",
  },
  {
    icon: "🏙️",
    title: "MAN",
    full: "Metropolitan Area Network",
    description:
      "A network designed to connect computers and networks across a city or large metropolitan area.",
    example: "City-wide network",
  },
  {
    icon: "📱",
    title: "PAN",
    full: "Personal Area Network",
    description:
      "A small network used around an individual, commonly connecting personal devices.",
    example: "Phone + Bluetooth device",
  },
];

const networkingDevices = [
  [
    "📡",
    "Router",
    "A router connects different networks and directs data packets toward their destination.",
  ],
  [
    "🔀",
    "Switch",
    "A switch connects devices within a network and forwards data to the appropriate device.",
  ],
  [
    "📶",
    "Modem",
    "A modem helps connect a local network or computer to an internet service.",
  ],
  [
    "🔁",
    "Repeater",
    "A repeater receives and regenerates signals to extend network communication distance.",
  ],
  [
    "🌉",
    "Bridge",
    "A bridge connects and helps manage communication between network segments.",
  ],
  [
    "🛡️",
    "Firewall",
    "A firewall monitors and controls network traffic according to security rules.",
  ],
];

const internetServices = [
  ["🌐", "World Wide Web", "Access websites and web pages through the Internet."],
  ["📧", "Email", "Send and receive electronic messages over the Internet."],
  ["💬", "Instant Messaging", "Communicate with people using online messaging services."],
  ["🎥", "Video Conferencing", "Communicate through live audio and video online."],
  ["☁️", "Cloud Services", "Store and access files and services over the Internet."],
  ["🛒", "E-Commerce", "Buy and sell products and services online."],
  ["🎓", "Online Learning", "Learn through websites, applications and online courses."],
  ["🏦", "Online Banking", "Access banking services through secure online systems."],
];

const importantTerms = [
  ["IP Address", "A numerical address used to identify a device on a network."],
  ["Domain Name", "A human-readable name used to access a website."],
  ["URL", "Uniform Resource Locator — the address of a resource on the web."],
  ["Browser", "Software used to access and view websites."],
  ["Server", "A computer or system that provides services or resources to other computers."],
  ["Client", "A device or application that requests services from a server."],
  ["Wi-Fi", "Wireless networking technology used to connect devices to a network."],
  ["HTTP", "Hypertext Transfer Protocol used for communication on the web."],
];

const networkAdvantages = [
  "Sharing files and resources",
  "Sharing printers and other devices",
  "Internet connection sharing",
  "Fast communication",
  "Centralized data access",
  "Easy collaboration",
];

const internetSafety = [
  "Use strong and unique passwords.",
  "Do not share passwords or OTPs with anyone.",
  "Check website addresses before entering sensitive information.",
  "Keep operating systems and applications updated.",
  "Avoid downloading files from unknown sources.",
  "Use secure Wi-Fi networks whenever possible.",
];

const revisionPoints = [
  "Internet is a worldwide network of interconnected computer networks.",
  "LAN covers a small geographical area.",
  "WAN covers a large geographical area.",
  "A router connects different networks.",
  "A switch connects devices within a network.",
  "An IP address identifies a device on a network.",
  "A browser is used to access websites.",
  "A server provides services or resources to clients.",
  "Wi-Fi provides wireless network connectivity.",
  "The World Wide Web is a service that operates over the Internet.",
];

export default function InternetNetworkingPage() {
  return (
    <main className="min-h-screen bg-white text-slate-800">

      {/* HEADER */}
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 lg:px-8">

          <Link href="/" className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-600 text-xl font-bold text-white shadow-lg">
              RI
            </div>

            <div>
              <h1 className="text-lg font-bold text-slate-900">
                Rishi Infotech
              </h1>
              <p className="text-xs font-medium text-blue-600">
                Learning Hub
              </p>
            </div>
          </Link>

          <nav className="hidden items-center gap-6 lg:flex">
            <Link href="/" className="font-medium hover:text-blue-600">
              Home
            </Link>

            <Link href="/learn" className="font-medium text-blue-600">
              Learn
            </Link>

            <Link href="/practice" className="font-medium hover:text-blue-600">
              Practice
            </Link>

            <Link href="/ai-hub" className="font-medium hover:text-blue-600">
              AI Hub
            </Link>

            <Link
              href="/developer-hub"
              className="font-medium hover:text-blue-600"
            >
              Developer
            </Link>
          </nav>

          <Link
            href="/contact"
            className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-blue-700"
          >
            Contact
          </Link>

        </div>
      </header>

      {/* HERO */}
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-700 via-blue-600 to-cyan-500">

        <div className="absolute -right-24 -top-24 h-80 w-80 rounded-full bg-white/10 blur-3xl" />
        <div className="absolute -bottom-40 -left-20 h-96 w-96 rounded-full bg-cyan-300/20 blur-3xl" />

        <div className="relative mx-auto max-w-7xl px-4 py-14 lg:px-8 lg:py-20">

          <Link
            href="/learn/computer"
            className="inline-flex rounded-full bg-white/15 px-4 py-2 text-sm font-semibold text-white backdrop-blur hover:bg-white/20"
          >
            ← Computer Chapters
          </Link>

          <div className="mt-8 flex items-center gap-5">

            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-3xl bg-white text-5xl shadow-xl">
              🌐
            </div>

            <div>
              <p className="font-semibold text-blue-100">
                CHAPTER 07
              </p>

              <h1 className="text-3xl font-black text-white sm:text-5xl">
                Internet & Networking
              </h1>
            </div>

          </div>

          <p className="mt-7 max-w-3xl text-lg leading-8 text-blue-50">
            Learn Internet basics, computer networks, LAN, WAN, networking
            devices, IP addresses, browsers, websites, online services and
            safe Internet usage.
          </p>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row">

            <a
              href="#internet"
              className="rounded-xl bg-white px-7 py-3.5 text-center font-bold text-blue-700 shadow-xl hover:scale-105"
            >
              🌐 Start Learning
            </a>

            <Link
              href="/practice/mcq"
              className="rounded-xl border-2 border-white/60 px-7 py-3.5 text-center font-bold text-white hover:bg-white hover:text-blue-700"
            >
              📝 Practice MCQs
            </Link>

          </div>

        </div>
      </section>

      {/* BREADCRUMB */}
      <div className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-4 text-sm lg:px-8">

          <Link href="/learn" className="text-blue-600 hover:underline">
            Learn
          </Link>

          <span className="mx-2 text-slate-400">/</span>

          <Link
            href="/learn/computer"
            className="text-blue-600 hover:underline"
          >
            Computer
          </Link>

          <span className="mx-2 text-slate-400">/</span>

          <span className="text-slate-500">
            Internet & Networking
          </span>

        </div>
      </div>

      {/* CONTENT */}
      <section id="internet" className="py-16">

        <div className="mx-auto max-w-7xl px-4 lg:px-8">

          <div className="grid gap-10 lg:grid-cols-[1fr_320px]">

            {/* MAIN CONTENT */}
            <article>

              {/* INTERNET INTRODUCTION */}
              <section>

                <p className="font-bold uppercase tracking-widest text-blue-600">
                  Chapter 07
                </p>

                <h2 className="mt-3 text-4xl font-black text-slate-900">
                  What is the Internet?
                </h2>

                <p className="mt-5 text-lg leading-8 text-slate-600">
                  The Internet is a worldwide system of interconnected
                  computer networks that allows devices and people to
                  communicate, share information and access online services.
                </p>

                <div className="mt-8 rounded-3xl bg-slate-900 p-8 text-white">

                  <div className="text-6xl">
                    🌍
                  </div>

                  <h3 className="mt-5 text-2xl font-black">
                    Internet = Global Network
                  </h3>

                  <p className="mt-3 leading-7 text-slate-300">
                    Computers, smartphones, servers and many other devices
                    can communicate with each other through interconnected
                    networks.
                  </p>

                </div>

              </section>

              {/* WHAT IS NETWORK */}
              <section id="network" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-cyan-600">
                  Networking Basics
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  What is a Computer Network?
                </h2>

                <p className="mt-5 leading-8 text-slate-600">
                  A computer network is a group of computers and other
                  devices connected together so they can communicate and
                  share information, resources and services.
                </p>

                <div className="mt-8 grid gap-5 md:grid-cols-3">

                  {[
                    ["💻", "Connected Devices", "Computers and devices communicate with each other."],
                    ["📂", "Resource Sharing", "Files, printers and other resources can be shared."],
                    ["💬", "Communication", "Users can communicate and exchange information."],
                  ].map(([icon, title, text]) => (
                    <div
                      key={title}
                      className="rounded-3xl border border-slate-200 p-6 shadow-sm"
                    >
                      <div className="text-4xl">{icon}</div>

                      <h3 className="mt-4 text-xl font-black text-slate-900">
                        {title}
                      </h3>

                      <p className="mt-3 text-sm leading-7 text-slate-600">
                        {text}
                      </p>
                    </div>
                  ))}

                </div>

              </section>

              {/* NETWORK TYPES */}
              <section id="network-types" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-indigo-600">
                  Classification
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Types of Computer Networks
                </h2>

                <div className="mt-8 grid gap-5 md:grid-cols-2">

                  {networkTypes.map((item) => (
                    <div
                      key={item.title}
                      className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
                    >

                      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-3xl">
                        {item.icon}
                      </div>

                      <h3 className="mt-5 text-2xl font-black text-slate-900">
                        {item.title}
                      </h3>

                      <p className="mt-1 text-xs font-bold uppercase tracking-wider text-blue-600">
                        {item.full}
                      </p>

                      <p className="mt-4 text-sm leading-7 text-slate-600">
                        {item.description}
                      </p>

                      <div className="mt-5 rounded-xl bg-slate-50 p-3 text-sm font-bold text-slate-700">
                        Example: {item.example}
                      </div>

                    </div>
                  ))}

                </div>

              </section>

              {/* NETWORK DEVICES */}
              <section id="devices" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-blue-600">
                  Hardware
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Networking Devices
                </h2>

                <p className="mt-5 leading-8 text-slate-600">
                  Networking devices help computers and other devices
                  communicate with each other and connect to networks.
                </p>

                <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">

                  {networkingDevices.map(([icon, title, description]) => (
                    <div
                      key={title}
                      className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"
                    >

                      <div className="text-4xl">
                        {icon}
                      </div>

                      <h3 className="mt-4 text-xl font-black text-slate-900">
                        {title}
                      </h3>

                      <p className="mt-3 text-sm leading-7 text-slate-600">
                        {description}
                      </p>

                    </div>
                  ))}

                </div>

              </section>

              {/* IP ADDRESS */}
              <section id="ip-address" className="mt-16 rounded-3xl bg-blue-50 p-8">

                <p className="font-bold uppercase tracking-widest text-blue-600">
                  Important Concept
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  What is an IP Address?
                </h2>

                <p className="mt-5 leading-8 text-slate-600">
                  An IP address is a numerical address used to identify a
                  device or network interface on a computer network.
                </p>

                <div className="mt-7 grid gap-4 sm:grid-cols-2">

                  {[
                    ["IPv4", "Common format using four groups of numbers."],
                    ["IPv6", "Newer format designed to provide a much larger address space."],
                  ].map(([title, text]) => (
                    <div
                      key={title}
                      className="rounded-2xl bg-white p-6 shadow-sm"
                    >
                      <h3 className="text-xl font-black text-blue-700">
                        {title}
                      </h3>

                      <p className="mt-3 text-sm leading-7 text-slate-600">
                        {text}
                      </p>
                    </div>
                  ))}

                </div>

              </section>

              {/* WEB */}
              <section id="web" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-cyan-600">
                  World Wide Web
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Internet, Websites and Browsers
                </h2>

                <p className="mt-5 leading-8 text-slate-600">
                  The World Wide Web is a collection of interconnected
                  websites and web resources accessed through the Internet.
                  A web browser allows users to open and interact with
                  these resources.
                </p>

                <div className="mt-8 grid gap-5 md:grid-cols-3">

                  {[
                    ["🌐", "Website", "A collection of related web pages and resources."],
                    ["🔗", "URL", "The address used to locate a resource on the web."],
                    ["🌍", "Browser", "Software used to access websites and web pages."],
                  ].map(([icon, title, text]) => (
                    <div
                      key={title}
                      className="rounded-3xl border border-cyan-100 bg-cyan-50 p-6"
                    >
                      <div className="text-4xl">{icon}</div>

                      <h3 className="mt-4 text-xl font-black text-slate-900">
                        {title}
                      </h3>

                      <p className="mt-3 text-sm leading-7 text-slate-600">
                        {text}
                      </p>
                    </div>
                  ))}

                </div>

              </section>

              {/* SERVICES */}
              <section id="services" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-indigo-600">
                  Online World
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Internet Services
                </h2>

                <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">

                  {internetServices.map(([icon, title, text]) => (
                    <div
                      key={title}
                      className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"
                    >
                      <div className="text-3xl">{icon}</div>

                      <h3 className="mt-3 font-black text-slate-900">
                        {title}
                      </h3>

                      <p className="mt-2 text-sm leading-6 text-slate-500">
                        {text}
                      </p>
                    </div>
                  ))}

                </div>

              </section>

              {/* ADVANTAGES */}
              <section id="advantages" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-blue-600">
                  Benefits
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Advantages of Computer Networking
                </h2>

                <div className="mt-8 grid gap-4 sm:grid-cols-2">

                  {networkAdvantages.map((item) => (
                    <div
                      key={item}
                      className="flex items-center gap-3 rounded-xl border border-slate-200 p-4"
                    >
                      <span className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 font-bold text-blue-600">
                        ✓
                      </span>

                      <span className="font-semibold text-slate-700">
                        {item}
                      </span>
                    </div>
                  ))}

                </div>

              </section>

              {/* SAFETY */}
              <section id="safety" className="mt-16 rounded-3xl bg-slate-900 p-8 text-white">

                <p className="font-bold uppercase tracking-widest text-cyan-400">
                  Online Safety
                </p>

                <h2 className="mt-3 text-3xl font-black">
                  Basic Internet Safety
                </h2>

                <p className="mt-4 leading-7 text-slate-300">
                  Safe Internet habits help protect personal information,
                  accounts and devices from common online threats.
                </p>

                <div className="mt-7 grid gap-4 sm:grid-cols-2">

                  {internetSafety.map((item) => (
                    <div
                      key={item}
                      className="flex gap-3 rounded-xl bg-white/5 p-4"
                    >
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-green-500 text-sm font-bold">
                        ✓
                      </span>

                      <span className="text-sm leading-6 text-slate-300">
                        {item}
                      </span>
                    </div>
                  ))}

                </div>

              </section>

              {/* TERMS */}
              <section id="terms" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-cyan-600">
                  Quick Revision
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Important Networking Terms
                </h2>

                <div className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">

                  {importantTerms.map(([term, description], index) => (
                    <div
                      key={term}
                      className={`grid grid-cols-1 gap-2 md:grid-cols-3 ${
                        index !== importantTerms.length - 1
                          ? "border-b border-slate-200"
                          : ""
                      }`}
                    >

                      <div className="p-5 font-black text-blue-600">
                        {term}
                      </div>

                      <div className="p-5 text-sm leading-6 text-slate-600 md:col-span-2">
                        {description}
                      </div>

                    </div>
                  ))}

                </div>

              </section>

              {/* REVISION */}
              <section id="revision" className="mt-16">

                <p className="font-bold uppercase tracking-widest text-blue-600">
                  Exam Revision
                </p>

                <h2 className="mt-3 text-3xl font-black text-slate-900">
                  Important Points
                </h2>

                <div className="mt-8 space-y-3">

                  {revisionPoints.map((item, index) => (
                    <div
                      key={item}
                      className="flex gap-4 rounded-xl border border-slate-200 bg-white p-4"
                    >

                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-black text-blue-600">
                        {index + 1}
                      </span>

                      <p className="text-sm leading-6 text-slate-600">
                        {item}
                      </p>

                    </div>
                  ))}

                </div>

              </section>

              {/* PRACTICE */}
              <section className="mt-16 rounded-3xl bg-gradient-to-br from-blue-700 to-cyan-500 p-8 text-white shadow-2xl md:p-10">

                <p className="font-bold uppercase tracking-widest text-blue-100">
                  Test Your Knowledge
                </p>

                <h2 className="mt-3 text-3xl font-black">
                  Ready for Internet & Networking MCQs?
                </h2>

                <p className="mt-4 max-w-2xl leading-7 text-blue-50">
                  Test your understanding of Internet basics, networking,
                  network devices, IP addresses and online services.
                </p>

                <div className="mt-7 flex flex-col gap-3 sm:flex-row">

                  <Link
                    href="/practice/mcq"
                    className="rounded-xl bg-white px-7 py-3.5 text-center font-bold text-blue-700 shadow-lg hover:scale-105"
                  >
                    📝 Start MCQ
                  </Link>

                  <Link
                    href="/practice/mock-tests"
                    className="rounded-xl border-2 border-white/60 px-7 py-3.5 text-center font-bold text-white hover:bg-white hover:text-blue-700"
                  >
                    🎯 Mock Test
                  </Link>

                </div>

              </section>

            </article>

            {/* SIDEBAR */}
            <aside className="lg:sticky lg:top-24 lg:h-fit">

              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">

                <h3 className="text-lg font-black text-slate-900">
                  Chapter Contents
                </h3>

                <div className="mt-5 space-y-2">

                  {[
                    ["01", "What is Internet?", "#internet"],
                    ["02", "Computer Network", "#network"],
                    ["03", "Network Types", "#network-types"],
                    ["04", "Networking Devices", "#devices"],
                    ["05", "IP Address", "#ip-address"],
                    ["06", "Web & Browsers", "#web"],
                    ["07", "Internet Services", "#services"],
                    ["08", "Network Advantages", "#advantages"],
                    ["09", "Internet Safety", "#safety"],
                    ["10", "Important Terms", "#terms"],
                    ["11", "Revision Points", "#revision"],
                  ].map(([number, title, href]) => (
                    <a
                      key={number}
                      href={href}
                      className="block rounded-xl bg-slate-50 px-4 py-3 text-sm font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-600"
                    >
                      <span className="mr-2 font-bold text-blue-600">
                        {number}.
                      </span>
                      {title}
                    </a>
                  ))}

                </div>

              </div>

              <div className="mt-5 rounded-3xl bg-blue-50 p-6">

                <h3 className="font-black text-blue-900">
                  Chapter Progress
                </h3>

                <div className="mt-5 h-2 overflow-hidden rounded-full bg-blue-200">
                  <div className="h-full w-2/3 rounded-full bg-blue-600" />
                </div>

                <p className="mt-3 text-xs text-blue-800">
                  Continue learning to complete this chapter.
                </p>

              </div>

            </aside>

          </div>
        </div>
      </section>

      {/* NAVIGATION */}
      <section className="border-t border-slate-200 bg-slate-50 py-10">

        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 px-4 sm:flex-row lg:px-8">

          <Link
            href="/learn/computer/storage"
            className="rounded-xl border border-slate-200 bg-white px-6 py-3 text-center font-bold text-slate-700 hover:border-blue-300 hover:text-blue-600"
          >
            ← Previous: Storage Devices
          </Link>

          <Link
            href="/learn/computer/cyber-security"
            className="rounded-xl bg-blue-600 px-6 py-3 text-center font-bold text-white hover:bg-blue-700"
          >
            Next: Cyber Security →
          </Link>

        </div>

      </section>

      {/* FOOTER */}
      <footer className="bg-slate-900 text-white">

        <div className="mx-auto max-w-7xl px-4 py-12 lg:px-8">

          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">

            <div>
              <h2 className="text-xl font-bold">
                Rishi Infotech
              </h2>

              <p className="mt-3 text-sm leading-6 text-slate-400">
                Learning, practice, AI tools and developer utilities
                in one place.
              </p>
            </div>

            <div>
              <h3 className="font-semibold">
                Learning
              </h3>

              <div className="mt-4 space-y-2 text-sm text-slate-400">

                <Link href="/learn" className="block hover:text-white">
                  All Subjects
                </Link>

                <Link
                  href="/learn/computer"
                  className="block hover:text-white"
                >
                  Computer
                </Link>

                <Link
                  href="/learn/physics"
                  className="block hover:text-white"
                >
                  Physics
                </Link>

              </div>
            </div>

            <div>
              <h3 className="font-semibold">
                Practice
              </h3>

              <div className="mt-4 space-y-2 text-sm text-slate-400">

                <Link href="/practice" className="block hover:text-white">
                  Practice Center
                </Link>

                <Link
                  href="/practice/mcq"
                  className="block hover:text-white"
                >
                  MCQ Quiz
                </Link>

                <Link
                  href="/practice/mock-tests"
                  className="block hover:text-white"
                >
                  Mock Tests
                </Link>

              </div>
            </div>

            <div>
              <h3 className="font-semibold">
                Tools
              </h3>

              <div className="mt-4 space-y-2 text-sm text-slate-400">

                <Link
                  href="/ai-hub"
                  className="block hover:text-white"
                >
                  AI Hub
                </Link>

                <Link
                  href="/developer-hub"
                  className="block hover:text-white"
                >
                  Developer Hub
                </Link>

                <Link
                  href="/dictionary"
                  className="block hover:text-white"
                >
                  Dictionary
                </Link>

              </div>
            </div>

          </div>

          <div className="mt-10 border-t border-slate-700 pt-6 text-center text-sm text-slate-500">
            © 2026 Rishi Infotech Learning Hub. All rights reserved.
          </div>

        </div>

      </footer>

    </main>
  );
}